...
export function validateVehicleGETRequest(req: Request, res: Response, next?: NextFunction) {
...
export function getVehicleStatus(req: Request, res: Response) {
  ...
  return res.status(200).json(vehicles[vin]);
}

export interface IStateDifference {
  lockState?: "locked"|"unlocked";
  blinking?: boolean;
}

export function setVehicleStatus(vin: string, difference: IStateDifference) {
  for (const key in difference) {
    if (difference.hasOwnProperty(key)) {
      const value = difference[key];
      switch (key) {
        case "lockState":
          Logger.info("Changing vehicle lockState to:", value);
          vehicles[vin].vehicleStatus.coreStatus = vehicles[vin].vehicleStatus.coreStatus.map((status) => {
            switch (status.key) {
              ...
              case "DOOR_REAR_RIGHT_LOCK_STATUS":
                status.value = value === "unlocked" ? "UNLOCKED" : "LOCKED";
                break;
              case "DOOR_IS_ALL_DOORS_LOCKED":
                status.value = value === "unlocked" ? "FALSE" : "TRUE";
                break;
            }
            return status;
          });
          break;
        case "blinking":
          Logger.info("Changing vehicle blinking state to:", value);
          break;
      }
    }
  }

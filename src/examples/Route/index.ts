...
import { getRoute, getTrips, getUserInformation, login, registerClient } from "../../commands";
import { addSpeedToWaypointList } from "../../utils";
...
async function run() {
  ...
  const waypoints = await getRoute(user.VIN, deviceId, tokens.access_token, trips[0].id.toString(), true);
  console.log(JSON.stringify(waypoints, undefined, 2));
